...
function enviarDadosDoadores() {
  //  const db_url = "https://hemocentro.ddns.net/cadastrar_doador";
  const db_url = "http://localhost/cadastrar_doador";
  const db_port = "5000";
  ...
  try {
    // Envia os dados para o servidor Flask `${db_url}:${db_port}`
    const response = fetch(127.0.0.1:5000, {
      method: "POST",
      headers: {
